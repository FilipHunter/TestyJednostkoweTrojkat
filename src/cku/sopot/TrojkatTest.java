package cku.sopot;

import org.junit.jupiter.api.Assertions;
import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.*;

class TrojkatTest {

    @Test
    void ujemnaWartoscPodstawaNiedozwolona() {
        Trojkat trojkat = new Trojkat(4,10);
        trojkat.setPodstawa(-10);
        Assertions.assertEquals(8, trojkat.getPodstawa());
    }

    @Test
    void ujemnaWartoscWysokoscNiedozowolna() {
        Trojkat trojkat = new Trojkat(4, 10);
        trojkat.setWysokosc(-5);
        Assertions.assertEquals(5, trojkat.getWysokosc());
    }

    @Test
    void wartosciKonstruktorUjemneNiedozwolone() {
        Trojkat trojkat = new Trojkat(-6, -12);
        Assertions.assertEquals(6, trojkat.getWysokosc());
        Assertions.assertEquals(12, trojkat.getPodstawa());
    }

    @Test
    void poprawneObliczeniePola() {
        Trojkat trojkat = new Trojkat(-8, -6);
        Assertions.assertEquals(24, trojkat.obliczPole());
    }
}